...
@section('title', $settingsLanguage->search_name.' - '.$settings->title.'')
@section('keyword', $settingsLanguage->search_name.','.$settings->keyword.'')
@section('description', $settingsLanguage->search_name.' - '.$settings->description.'')
    ...
    <div class="row">
      <div class="inner">
        <ul>
          ...
          <li class="category13"><strong>{{ $settingsLanguage->search_name }}</strong></li>
        </ul>
      </div>
    </div>
  </div>
    ...
    <div class="row">
      <div class="col-main col-sm-9 col-sm-push-3">
        <article class="col-main">
          <div class="page-title" style="padding-left:20px;"><b>{{ Lang::get('content.search_word') }} : </b> {{ $searchText }} <span style="float:right; color:#989898; font-style:italic;">{{ Lang::get('content.search_total') }} <b>{{ count($products) }}</b> {{ Lang::get('content.search_product') }}</span></div>
  ...
  <div class="container">
    <div class="row">
      <div class="col-main col-sm-12">
        <article class="col-main">
          <div class="page-title"><span class="glyphicon glyphicon-search"></span> <b>{{ Lang::get('content.search_word') }} : </b> {{ $searchText }} <span style="float:right; color:#989898; font-style:italic;">{{ Lang::get('content.search_total') }} <b>{{ count($pages) }}</b> {{ Lang::get('content.search_page') }}</span></div>
          <hr>
              @foreach($pages as $page)
              <div class="search-list">
              <a href="{{ Sef::pagelang($page->sef_url,$languageActive) }}">              	
              <b><span class="glyphicon glyphicon-chevron-right"></span>{{ $page->title }}</b><br />
              <span class="info">{{ $page->description }}</span><br />
          	  </a>              
          	  </div>
              @endforeach
          </article>
